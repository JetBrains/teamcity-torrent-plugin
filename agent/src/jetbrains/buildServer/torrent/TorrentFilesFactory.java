 ...
 */
public class TorrentFilesFactory {
  private final static Logger LOG = Logger.getInstance(TorrentFilesFactory.class.getName());

  private final File myTorrentFilesDir;
  private final Random myRandom;
  private final TorrentConfiguration myConfiguration;

  public TorrentFilesFactory(@NotNull BuildAgentConfiguration agentConfiguration,
  ...
  public File createTorrentFile(@NotNull File srcFile) {
    ...
    try {
      File torrentFile = getTorrentFile(srcFile);
  ...
  @NotNull
  private File getTorrentFile(@NotNull File srcFile) throws IOException {
    long hash = Math.abs(srcFile.getAbsolutePath().hashCode());
    long dirIdx = hash % 10;
